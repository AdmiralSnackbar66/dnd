...
Status:
  - ⏳
OneLiner: The students turn up for their first yeat at Strixhaven and encounter an unlikely surpisde during Orientation
timelines:
  - journal
tags:
  - journal
...
```dataview
TABLE WITHOUT ID link(file.name) AS "Character Name",Player,race,Class, hp, ac, modifier, pasperc As "Passive Perception (WIS)", choice(Field1, "☑", "☐") as Present
from "Campaigns"
where (Role = "Player") 
where (Status = "Active") 
```
